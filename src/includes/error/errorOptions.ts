...
ErrorOptions.BAD_REQUEST = {
  error_code : 'BAD_REQUEST',
  message    : 'Bad request.',
  code       : 400,
};

ErrorOptions.generator.paramRequired = ({
  param_name,
}: {
  param_name: string;
}) => ({
...
ErrorOptions.generator.badlyFormattedDateString = ({
  param_name,
}: {
  param_name: string;
}) => {
...
ErrorOptions.generator.badlyFormattedEmail = ({
  param_name,
}: {
  param_name: string;
}) => {
...
ErrorOptions.generator.unallowedValue = ({
  ...
  return {
    ...
    message    : `Validation failed for field: ${param_name}. Value is not elligible for this field. Must be one of ${white_list}`,
    code       : 400,
  };
};
