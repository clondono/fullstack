...
const decrypt = function(encrypted_string: string): any {
  const buffer_data = bs58.decode(encrypted_string);
  const iv_data = buffer_data.slice(0, IV_LENGTH);
  const encrypted_data = buffer_data.slice(
    IV_LENGTH,
    buffer_data.length - HMAC_LENGTH,
  );
  const hmac_data = buffer_data.slice(
    buffer_data.length - HMAC_LENGTH,
    buffer_data.length,
  );
