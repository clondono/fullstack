...
const { randomBytesAsync, createCipheriv, createHmac }: any =
...
const encrypt = async(data: { [x: string]: any }): Promise<string> => {
  ...
  const hmacData = createHmac(HMAC_ALGORITHM, HMAC_KEY)
    .update(bufferEncryptedData)
    .digest();
