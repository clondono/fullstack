...
interface UpdateGridSettingsParams {
  user_id: string;
}

const updateGridSettings = async ({ user_id }: UpdateGridSettingsParams) => {
  const old_user = await UsersGet.getByUserId({ user_id });

  const bindings: UpdateGridSettingsParams = {
    user_id,
  };
...
interface UpdatePersonaalDetailsParams {
  user_id: string;
  first_name?: string;
  last_name?: string;
  company?: string;
