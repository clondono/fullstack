...
const insertUserInvite = async({
  ...
  const queries: SqlQueryParams[] = [
    { query: CREATE_INVITE, bindings: { email, token } },
  ];
  try {
    ...
    if (e.code === '23505') {
      ...
      const match = await PgQuery.query([
        { query: query_match, bindings: bindings_match },
      ]);
      token = match[0].token;
    } else {
      logger.error(e);
...
const isValidEmailForInvite = async({ email }: { email: string }) => {
  ...
  const rows = await PgQuery.query([
    { query: USER_EXISTS_QUERY, bindings: bindings },
  ]);
  if (!_.isEmpty(rows)) {
...
const sendInviteEmail = async({
  ...
  const prepend =
    endpoint.indexOf('https://') === -1 && !endpoint.includes('localhost')
      ? 'https://'
      : '';
